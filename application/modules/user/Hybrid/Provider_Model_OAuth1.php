...
/**
 * To implement an OAuth 1 based service provider, User_Hybrid_Provider_Model_OAuth1
 * can be used to save the hassle of the authentication flow. 
 * 
 * Each class that inherit from User_Hybrid_Provider_Model_OAuth1 have to implemenent
 * at least 2 methods:
 *   User_Hybrid_Providers_{provider_name}::initialize()     to setup the provider api end-points urls
 *   User_Hybrid_Providers_{provider_name}::getUserProfile() to grab the user profile
 *
 * User_Hybrid_Provider_Model_OAuth1 use OAuth1Client v0.1 which can be found on
 * Hybrid/thirdparty/OAuth/OAuth1Client.php
 */
class User_Hybrid_Provider_Model_OAuth1 extends User_Hybrid_Provider_Model
{
	...
	*/
	function initialize()
	{
		...
		require_once User_Hybrid_Auth::$config["path_libraries"] . "OAuth/OAuth.php";
		require_once User_Hybrid_Auth::$config["path_libraries"] . "OAuth/OAuth1Client.php";
		...
		if( isset( User_Hybrid_Auth::$config["proxy"] ) ){
			$this->api->curl_proxy = User_Hybrid_Auth::$config["proxy"];
		}
	}
	...
	*/
	function loginBegin()
	{
		...
		User_Hybrid_Auth::redirect( $this->api->authorizeUrl( $tokens ) );
	}
