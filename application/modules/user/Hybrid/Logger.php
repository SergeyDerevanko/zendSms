 ...
 */
class User_Hybrid_Logger
{
	function __construct()
	{
		// if debug mode is set to true, then check for the writable log file
		if ( User_Hybrid_Auth::$config["debug_mode"] ){
			if ( ! file_exists( User_Hybrid_Auth::$config["debug_file"] ) ){
				throw new Exception( "'debug_mode' is set to 'true', but the file " . User_Hybrid_Auth::$config['debug_file'] . " in 'debug_file' does not exit.", 1 );
			}

			if ( ! is_writable( User_Hybrid_Auth::$config["debug_file"] ) ){
	...
	public static function debug( $message, $object = NULL )
	{
		if( User_Hybrid_Auth::$config["debug_mode"] ){
			...
			file_put_contents( 
				User_Hybrid_Auth::$config["debug_file"],
