...
/**
 * To implement an OpenID based service provider, User_Hybrid_Provider_Model_OpenID
 * can be used to save the hassle of the authentication flow. 
 * 
 * Each class that inherit from User_Hybrid_Provider_Model_OAuth2 have only to define
 * the provider identifier : <code>public $openidIdentifier = ""; </code>
 *
 * User_Hybrid_Provider_Model_OpenID use LightOpenID lib which can be found on
 * Hybrid/thirdparty/OpenID/LightOpenID.php
 */
class User_Hybrid_Provider_Model_OpenID extends User_Hybrid_Provider_Model
{
	...
	*/
	function initialize()
	{
		...
		require_once User_Hybrid_Auth::$config["path_libraries"] . "OpenID/LightOpenID.php";
		
		// An error was occurring when proxy wasn't set. Not sure where proxy was meant to be set/initialized.
		User_Hybrid_Auth::$config['proxy'] = isset(User_Hybrid_Auth::$config['proxy'])?User_Hybrid_Auth::$config['proxy']:'';
		
		$this->api = new LightOpenID( parse_url( User_Hybrid_Auth::$config["base_url"], PHP_URL_HOST), User_Hybrid_Auth::$config["proxy"] );
	}
	...
	*/
	function loginBegin()
	{
		...
		User_Hybrid_Auth::redirect( $this->api->authUrl() );
	}
	...
	*/
	function loginFinish()
	{
		...
		User_Hybrid_Auth::storage()->set( "hauth_session.{$this->providerId}.user", $this->user );
	}
	...
	*/
	function getUserProfile()
	{
		...
		$this->user = User_Hybrid_Auth::storage()->get( "hauth_session.{$this->providerId}.user" ) ;
