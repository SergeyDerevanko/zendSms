...
/**
 * User_Hybrid_Provider_Adapter is the basic class which User_Hybrid_Auth will use
 * to connect users to a given provider. 
 * 
 * Basically User_Hybrid_Provider_Adapterwill create a bridge from your php
 * application to the provider api.
 * 
 * User_Hybrid_Auth will automatically load User_Hybrid_Provider_Adapter and create
 * an instance of it for each authenticated provider.
 */
class User_Hybrid_Provider_Adapter
{
	...
	*/
	...
	{
		User_Hybrid_Logger::info( "Enter User_Hybrid_Provider_Adapter::factory( $id )" );
		...
		if( isset( $this->config["wrapper"] ) && is_array( $this->config["wrapper"] ) ){
			...
			require_once User_Hybrid_Auth::$config["path_providers"] . $this->id . ".php" ;

			$this->wrapper = "User_Hybrid_Providers_" . $this->id;
		}
	...
	/**
	* User_Hybrid_Provider_Adapter::login(), prepare the user session and the authentication request
	* for index.php
	*/
	function login()
	{
		User_Hybrid_Logger::info( "Enter User_Hybrid_Provider_Adapter::login( {$this->id} ) " );

		if( ! $this->adapter ){
			throw new Exception( "User_Hybrid_Provider_Adapter::login() should not directly used." );
		}

		// clear all unneeded params
		foreach( User_Hybrid_Auth::$config["providers"] as $idpid => $params ){
			User_Hybrid_Auth::storage()->delete( "hauth_session.{$idpid}.hauth_return_to"    );
			User_Hybrid_Auth::storage()->delete( "hauth_session.{$idpid}.hauth_endpoint"     );
			User_Hybrid_Auth::storage()->delete( "hauth_session.{$idpid}.id_provider_params" );
		}
		...
		$User_Hybrid_AUTH_URL_BASE = User_Hybrid_Auth::$config["base_url"];
		...
		$this->params["login_start"] = $User_Hybrid_AUTH_URL_BASE . ( strpos( $User_Hybrid_AUTH_URL_BASE, '?' ) ? '&' : '?' ) . "hauth.start={$this->id}&hauth.time={$this->params["hauth_time"]}";

		# for default HybridAuth endpoint url hauth_login_done_url
		# 	auth.done   required  the IDp ID
		$this->params["login_done"]  = $User_Hybrid_AUTH_URL_BASE . ( strpos( $User_Hybrid_AUTH_URL_BASE, '?' ) ? '&' : '?' ) . "hauth.done={$this->id}";

		User_Hybrid_Auth::storage()->set( "hauth_session.{$this->id}.hauth_return_to"    , $this->params["hauth_return_to"] );
		User_Hybrid_Auth::storage()->set( "hauth_session.{$this->id}.hauth_endpoint"     , $this->params["login_done"] );
		User_Hybrid_Auth::storage()->set( "hauth_session.{$this->id}.id_provider_params" , $this->params );

		// store config to be used by the end point 
		User_Hybrid_Auth::storage()->config( "CONFIG", User_Hybrid_Auth::$config );

		// move on
		User_Hybrid_Logger::debug( "User_Hybrid_Provider_Adapter::login( {$this->id} ), redirect the user to login_start URL." );

		User_Hybrid_Auth::redirect( $this->params["login_start"] );
	}
	...
	*/ 
	...
	{
		User_Hybrid_Logger::info( "Enter User_Hybrid_Provider_Adapter::$name(), Provider: {$this->id}" );
		...
		if ( ! method_exists( $this->adapter, $name ) ){
			throw new Exception( "Call to undefined function User_Hybrid_Providers_{$this->id}::$name()." );
		}
	...
	*/
	public function getAccessToken()
	{
		if( ! $this->adapter->isUserConnected() ){
			User_Hybrid_Logger::error( "User not connected to the provider." );
	...
	*/
	...
	{ 
		...
		$callback_url = User_Hybrid_Auth::storage()->get( "hauth_session.{$this->id}.hauth_return_to" );

		// remove some unneed'd stored data 
		User_Hybrid_Auth::storage()->delete( "hauth_session.{$this->id}.hauth_return_to"    );
		User_Hybrid_Auth::storage()->delete( "hauth_session.{$this->id}.hauth_endpoint"     );
		User_Hybrid_Auth::storage()->delete( "hauth_session.{$this->id}.id_provider_params" );

		// back to home
		User_Hybrid_Auth::redirect( $callback_url );
	}
	...
	*/
	...
	{ 
		if( isset( User_Hybrid_Auth::$config["providers"][$id] ) ){
			return User_Hybrid_Auth::$config["providers"][$id];
		}
